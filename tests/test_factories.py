...
def test_create_credentials_with_spreadsheet_scope(mock_from_file):
    ...
    manager = GoogleCredentialsFactory(path=Path("/fake/path/to/creds.json"), scope=GoogleCredentialsScope.SPREADSHEETS)
...
def test_create_credentials_with_no_scope(mock_from_file):
    ...
    manager = GoogleCredentialsFactory(path=Path("/fake/path/to/creds.json"), scope=None)
